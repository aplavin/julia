        ...
        elseif isa(stmt, GotoIfNot)
            ...
            elseif condval === false
                ...
                if isa(condt, Conditional)
                    changes_else = conditional_changes(changes_else, condt.elsetype, condt.slot)
                    changes      = conditional_changes(changes,      condt.thentype, condt.slot)
                end
...
function conditional_changes(changes::VarTable, @nospecialize(typ), slot::Int)
    vtype = changes[slot]
    oldtyp = vtype.typ
    ...
    if ignorelimited(typ) ⊑ ignorelimited(oldtyp)
        ...
        return StateUpdate(SlotNumber(slot), VarState(typ, vtype.undef), changes, true)
    end
    return changes
end
