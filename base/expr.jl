...
from an earlier meta effect. For example, `:pure !:nothrow` indicates that while
the call is generally pure, it may however throw.

## Comparison to @pure
`@assume_effects :consistent :effect_free :terminates_globally` is similar to [`@pure`](@ref) with the primary
...
`@pure` should always be at least `:consistent :effect_free :terminates_globally`.
